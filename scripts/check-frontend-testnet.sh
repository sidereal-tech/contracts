...
load_addresses() {
  if [[ -n "$PROOF_FILE" ]]; then
    ...
    AMM="$(proof_value AMM)"
    UNDERLYING="$(proof_value UNDERLYING)"
    YIELD_SOURCE_FROM_FILE="$(proof_value YIELD_SOURCE)"
    BLEND_POOL_FROM_FILE="$(proof_value BLEND_POOL)"
  else
    ...
    UNDERLYING="$(manifest_value underlying)"
    YIELD_SOURCE_FROM_FILE="$(manifest_value yield_source)"
    BLEND_POOL_FROM_FILE="$(manifest_value blend_pool)"
  fi
}
...
export NEXT_PUBLIC_YIELD_SOURCE_KIND="${NEXT_PUBLIC_YIELD_SOURCE_KIND:-${YIELD_SOURCE_FROM_FILE:-mock}}"
export NEXT_PUBLIC_YIELD_SOURCE_NAME="${NEXT_PUBLIC_YIELD_SOURCE_NAME:-$([[ "$NEXT_PUBLIC_YIELD_SOURCE_KIND" == "blend" ]] && printf 'Blend v2 USDC pool' || printf 'Simulated testnet rate')}"
export NEXT_PUBLIC_YIELD_SOURCE_POOL_ADDRESS="${NEXT_PUBLIC_YIELD_SOURCE_POOL_ADDRESS:-${BLEND_POOL_FROM_FILE:-}}"
export NEXT_PUBLIC_YIELD_SOURCE_RESERVE_ADDRESS="${NEXT_PUBLIC_YIELD_SOURCE_RESERVE_ADDRESS:-${UNDERLYING:-}}"
export NEXT_PUBLIC_YIELD_SOURCE_URL="${NEXT_PUBLIC_YIELD_SOURCE_URL:-$([[ "$NEXT_PUBLIC_YIELD_SOURCE_KIND" == "blend" ]] && printf 'https://docs.blend.capital/' || printf '')}"
...
NEXT_PUBLIC_YIELD_SOURCE_KIND="$NEXT_PUBLIC_YIELD_SOURCE_KIND"
NEXT_PUBLIC_YIELD_SOURCE_NAME="$NEXT_PUBLIC_YIELD_SOURCE_NAME"
NEXT_PUBLIC_YIELD_SOURCE_POOL_ADDRESS="$NEXT_PUBLIC_YIELD_SOURCE_POOL_ADDRESS"
NEXT_PUBLIC_YIELD_SOURCE_RESERVE_ADDRESS="$NEXT_PUBLIC_YIELD_SOURCE_RESERVE_ADDRESS"
NEXT_PUBLIC_YIELD_SOURCE_URL="$NEXT_PUBLIC_YIELD_SOURCE_URL"
