...
| Maturity-freeze timing (Blend keeps accruing past maturity) | The freeze never reads a live post-maturity rate: it pins the last rate observed at or before maturity. Every mutating op records an observation, and the permissionless `observe_rate` poke lets a keeper or YT holder refresh it right up to maturity. The unobserved tail deterministically favors PT (senior), never a race |
