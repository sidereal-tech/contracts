...
redeem, and recombine. The invariant is maintained by construction and checked
off-chain: a 10k-step random property test verifies the full invariant (PT plus
YT) holds at every step, and external reads (`escrowed_sy`, the SY exchange
rate, PT total supply) let anyone re-verify it against the live deployment.

No entrypoint gates on escrow coverage on-chain. An earlier design asserted
`escrow_sy * R / WAD >= pt_supply` after every mutating call and rejected with
`Insolvent`, but with a Blend-derived rate the escrow has zero coverage slack
at mint (split floors the face), and a redeem can tick the rate down by a
sub-stroop rounding notch because Blend's bToken burn rounds in the pool's
favor. Gating on coverage turned that dust regression into a frozen market:
every claim reverted, even ones that owed nothing. The `Insolvent` error code
is retired (kept reserved), and shortfalls are priced instead of blocked.

**Insolvency (negative yield).** If the SY rate regresses (a slash, a loss) the
escrow may no longer cover all PT principal. The protocol prices the shortfall
rather than blocking or racing it:

- PT redemption and recombine are capped to the holder's pro-rata share of
  escrow, `escrow_shares * pt_amount / pt_supply`. PT holders share the
  shortfall pro-rata; capping preserves the escrow/PT ratio so later redeemers
  are not disadvantaged. When solvent, this equals full principal.
- YT claims are never gated on coverage. Regression safety lives in the settle
  math: a claim pays zero unless the rate has risen past the holder's
...
**Known limitation: YT route rounding dust.** The buy route rounds the SY split
up so the tokenizer's floor-rounded mint covers the quoted YT amount. At a
non-par SY rate this can leave a matched PT/YT dust pair in pool custody instead
of sending it to the trader. The sell route can also return one fewer SY share
than PT/YT face because recombine floors `amount * WAD / rate`. A direct dust
recombine fix was tested and rejected because the dust is worth less than one
SY share, and the tokenizer rejects a zero-value recombine. The accepted v1
behavior is to keep matched dust pool-side or burn the matched pair, reconcile
reserves, and cover both windows with integration tests. A later redesign needs
split resizing, a dust accumulator, a pool self-sweep entrypoint, or tokenizer
support for sub-share principal.
...
- Used by external integrators (a lending protocol that wants to use PT as collateral, for instance) to get a manipulation-resistant price. Integrators must gate on `twap_warming_up()` — the accumulator is only as good as the history behind it, and it reports honestly when there isn't one.
...
Each swap calls `sync_twap()` which performs:

```
elapsed    = current_time - last_observation_time
weight     = elapsed / window_size  // capped at 1.0
twap_apy   = twap_apy * (1 - weight) + prevailing_apy * weight
last_observation_time = current_time
```

`prevailing_apy` is the implied rate recorded **before** this trade — the rate
that actually held over `elapsed`. The post-trade rate is stored, not
accumulated; it gets its weight from whichever call closes the interval it goes
on to prevail over.

That distinction is the whole security property, and getting it backwards is a
live exploit rather than an inaccuracy. Weighting the *post-trade* rate by
`elapsed / window` means an attacker who waits until one second short of the
window and then trades hands their own dislocation a weight of ~0.999 — the
oracle snaps to a price they chose — and because a same-timestamp reversal is a
zero-length interval, they unwind in the same transaction and pay only the
round-trip fee. Measured against the deployed configuration before the fix, that
cost 0.10% of the pool to move the reported APY from 764 to 1747 bps, with
`twap_warming_up()` still reporting false. Corrected in the August 2026 audit
(finding H3); the regression tests assert both directions — that a 600× larger
same-instant reversal cannot move the oracle further, and that a dislocation
actually *held* over time does reach it.
...
| YT route rounding dust (§4.3) | Accepted v1 limitation: matched PT/YT dust never reaches the trader, sell-route recombine burns matched pairs, reserves are reconciled, and both dust windows are bounded by integration tests; deeper fixes require split resizing, a dust accumulator, pool self-sweep, or tokenizer redesign |
| Maturity-freeze timing (Blend keeps accruing past maturity) | The freeze never reads a live post-maturity rate: it pins the last rate observed at or before maturity. Every mutating operation records an observation, including direct YT transfers and burns, which resolve their settle rate through the tokenizer's `observe_rate` so YT can never bank yield at a rate the freeze did not see. The permissionless poke lets a keeper or YT holder refresh the observation right up to maturity; the unobserved tail deterministically favors PT (senior), never a race |
